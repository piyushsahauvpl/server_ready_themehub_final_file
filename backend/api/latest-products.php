<?php
/**
 * Public Latest Products API
 * Endpoint: /api/latest-products.php
 * Returns latest products for homepage (public access)
 */
...
try {
    ...
    $checkColumn = $conn->query("SHOW COLUMNS FROM products LIKE 'is_latest'");
    ...
    // Get latest products (only approved ones)
    $query = "SELECT 
                p.*,
              ...
              WHERE p.is_latest = 1 AND p.status = 'approved'
    ...
    error_log("Latest products API error: " . $e->getMessage());
    closeDBConnection($conn);
